...
const _Login = async (email, password, url, setErr) => {
    const data = {
        email: email,
        password: password,
    };
...
export const Login = ({ onSubmit }) => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (isCustomer) {
            const customer = await _Login(email, password, '/api/customers/login', setErr);
            onSubmit(customer._id);
            navigate(`/customer/${customer._id}`);
        } else {
            const restaurant = await _Login(email, password, '/api/restaurants/login', setErr);
